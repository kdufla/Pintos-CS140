...
* `struct thread* th` - ამ სემაფორაზე მომლოდინე ნაკადზე მიმთითებელი.
...
ამ სინქრონიზაციის მექანიზმის შემთხვევაში ყოველ ნაკადზე, რომელიც ელოდება ამ *condition variable*-ს, ხდება `semaphore_elem`-ის შექმნა, რომელიც თავის თავში ინახავს `semaphore`-ს, `list_elem`-სა და `thread* th`-ს. ყოველი `condition_wait`-ს დროს ხდება ერთი `semaphore_elem`-ის ინიციალიზაცია და იგი ემატება *condition*-ზე მომლოდინე ელემენტების სიაში. `condition_signal`-ი ხდება პრიორიტეტების მიხედვით. ამის მისაღწევად ყოველი სიგნალის წინ ხდება მომლოდინე `semaphore_elem`-ის დალაგება მათ სტრუქტურაში არსებული ნაკადების პრიორიტეტების მიხედვით. ამისთვის ვიყენებთ `sema_cmp` ფუნქციას, რომელიც ორი `semaphore_elem`-ის შესადარებლად იღებს მათ შესაბამის ცვლად `th`-ს და ამ ორი ცვლადის `priority`-ებს ადარებს.

## სინქრონიზაცია

`lock_acquire`-ში შეიძლება მოხდეს *race condition*, რადგან ამ დროს ხდება დონაცია, რაც ნიშნავს სხვა ნაკადის სტრუქტურაში `priority`-ს შეცვლას. რამდენიმე ნაკადმა ერთად რომ ვერ შეძლოს ამის გაკეთება, ამისთვის დროებით უნდა შეჩერდეს *scheduling*-ი და მიმდინარე ნაკადს მიეცეს დონაციის დამთავრების საშუალება. მეორე *race condition* შეიძლება მოხდეს `sem-up`-ში, როცა ერთსა და იმავე სემაფორაზე შეეცდებიან წვდომას და ამასაც იგივე მიდგომით ვაგვარებთ. ზუსტად იგივეა `lock_release`-ში – დონაციის დაბურუნება და ძველი დონაციის ან საკუთარი პრიორიტეტის აღდგენა ისეთ მონაცემებს ეხება, რასთან არასინქრონიზებულად შეხებამაც მარტივად შეიძლება გამოიწვიოს *race condition*.

## რატომ ავირჩიეთ ეს მიდგომა

*scheduler*-ის არჩევის მთავარი მიზეზი სიმარტივე იყო. არსებობს უფრო სწრაფი გზებიც, მაგალითად წითელი-შავი ხე, მაგრამ მისი კარგი იმპლემენტაცია ბევრად მეტ სირთულეებთანაა დაკავშირებული და ამ პროექტის მოთხოვნებს შედარებით ნელი *linked-list*-იც აკმაყოფილებს ელემენტების *ordered* ჩამატებით პრიორიტეტების ზრდადობის მიხედვით. დონაციისთვის არჩეული რეკურსია სანდო გზაა და გარანტირებულად ჩაიტანს ბოლომდე პრიორიტეტს. უფრო პრობლემატური არის გადაწერილი დონაციის აღდგენა, რასაც `lock_release`-ში ყველა `lock`-ზე გადაყოლით ვაგვარებთ. ამ ამოცანის ამოხსნისთვის სხვა გზებიც არსებობს, მაგრამ ემპირიული გამოცდილებით ვხედავთ, რომ ერთი შეედვით დიდი ოპერაცია - ორი ჩადგმული *for* ციკლი - რეალურად არც ისე დიდი ხდება და სამაგიეროდ სხვა მიდგომებთან შედარებით მეხსიერებას ძალიან ცოტას ხარჯავს.
